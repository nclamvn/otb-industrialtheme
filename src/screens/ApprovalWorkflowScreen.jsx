...
const ApprovalWorkflowScreen = ({ darkMode = false }) => {
  ...
  const handleDelete = async (id) => {
    const reason = window.prompt('Are you sure? Type "delete" to confirm:');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`backdrop-blur-xl rounded-2xl shadow-xl border p-6 ${
        darkMode ? 'bg-[#121212]/95 border-[#2E2E2E]' : 'bg-gradient-to-br from-white to-[rgba(215,183,151,0.1)] border-[#2E2E2E]/20'
      }`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-xl ${darkMode ? 'bg-[rgba(215,183,151,0.1)]' : 'bg-[rgba(215,183,151,0.15)]'}`}>
            ...
            <div>
              ...
              <p className={`text-sm font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>
                Configure approval steps for each brand
              </p>
            </div>
          </div>
      ...
      <div className={`rounded-xl border shadow-sm p-4 ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm font-semibold font-['Montserrat'] ${darkMode ? 'text-[#F2F2F2]' : 'text-[#0A0A0A]'}`}>Brand</span>
            <select
              ...
              className={`px-3 py-2 border rounded-lg text-sm font-['Montserrat'] transition-all focus:outline-none focus:ring-2 focus:ring-[#D7B797] ${
                darkMode
                  ...
                  : 'bg-white border-[#2E2E2E]/20 text-[#0A0A0A]'
              }`}
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                darkMode ? 'text-[#999999] hover:text-[#D7B797] hover:bg-[rgba(215,183,151,0.08)]' : 'text-[#666666] hover:text-[#8A6340] hover:bg-[rgba(215,183,151,0.15)]'
              }`}
            >
          ...
          {/* View Toggle */}
          <div className={`flex items-center gap-1 p-1 rounded-lg ${darkMode ? 'bg-[#1A1A1A]' : 'bg-[rgba(215,183,151,0.1)]'}`}>
            <button
      ...
      {loading ? (
        <div className="p-16 text-center">
          ...
          <p className={`text-sm font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Loading workflow steps...</p>
        </div>
      ) : viewMode === 'table' ? (
        /* Table View */
          ...
          <table className="w-full">
            <thead>
              <tr className={darkMode ? 'bg-[#1A1A1A]' : 'bg-[rgba(215,183,151,0.1)]'}>
                <th className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Brand</th>
                <th className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider font-['Montserrat'] w-20 ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Step</th>
                <th className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Role / User</th>
                <th className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Description</th>
                <th className={`px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider font-['Montserrat'] w-32 ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {steps.length === 0 ? (
                <tr>
                  <td colSpan="5" className={`px-4 py-16 text-center text-sm font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>
                    No workflow steps configured. Click "Add Step" to create one.
                  </td>
                </tr>
              ) : (
                steps.map((step) => (
                  <tr key={step.id} className={`border-t transition-colors ${
                    darkMode ? 'border-[#2E2E2E] hover:bg-[rgba(215,183,151,0.04)]' : 'border-[#2E2E2E]/10 hover:bg-[rgba(215,183,151,0.06)]'
                  }`}>
                  ...
                  <div key={step.id} className="flex items-center gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 ${
                        darkMode ? 'bg-[rgba(215,183,151,0.1)] border-[#D7B797]' : 'bg-[rgba(215,183,151,0.15)] border-[#D7B797]'
                      }`}>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setIsModalOpen(false)}>
          <div
            className={`rounded-2xl w-full max-w-md p-6 m-4 shadow-2xl ${
              darkMode ? 'bg-[#121212] border border-[#2E2E2E]' : 'bg-white border border-[#2E2E2E]/20'
            }`}
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${darkMode ? 'hover:bg-[#1A1A1A] text-[#999999]' : 'hover:bg-[rgba(215,183,151,0.15)] text-[#666666]'}`}
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {/* Brand */}
              <div>
                <label className={`block text-xs font-medium mb-1.5 font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Brand</label>
                <select
                  ...
                  className={`w-full px-3 py-2.5 border rounded-lg text-sm font-['Montserrat'] focus:outline-none focus:ring-2 focus:ring-[#D7B797] disabled:opacity-50 ${
                    darkMode ? 'bg-[#1A1A1A] border-[#2E2E2E] text-[#F2F2F2]' : 'bg-white border-[#2E2E2E]/20 text-[#0A0A0A]'
                  }`}
                >
              ...
              {/* Step Number */}
              <div>
                <label className={`block text-xs font-medium mb-1.5 font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Step Number</label>
                <input
                  type="number"
                  min="1"
              ...
              {/* Role */}
              <div>
                <label className={`block text-xs font-medium mb-1.5 font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Role</label>
                      ...
                      className={`px-3 py-2 text-sm font-['Montserrat'] rounded-lg border transition-colors ${
                        formData.roleCode === role.code
                          ...
                          : darkMode
                            ...
                            : 'border-[#2E2E2E]/20 text-[#666666] hover:border-[rgba(215,183,151,0.4)]'
                      }`}
                    >
                      {role.name}
                    </button>
              ...
              {/* Description */}
              <div>
                <label className={`block text-xs font-medium mb-1.5 font-['Montserrat'] ${darkMode ? 'text-[#999999]' : 'text-[#666666]'}`}>Description (Optional)</label>
                <input
                  type="text"
                  value={formData.description}
                  onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}
                  placeholder="e.g., Review budget allocation"
                  className={`w-full px-3 py-2.5 border rounded-lg text-sm font-['Montserrat'] focus:outline-none focus:ring-2 focus:ring-[#D7B797] ${
                    darkMode ? 'bg-[#1A1A1A] border-[#2E2E2E] text-[#F2F2F2] placeholder-[#666666]' : 'bg-white border-[#2E2E2E]/20 text-[#0A0A0A] placeholder-[#999999]'
                  }`}
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className={`flex-1 px-4 py-2.5 border rounded-lg font-medium text-sm font-['Montserrat'] transition-colors ${
                  darkMode ? 'border-[#2E2E2E] text-[#999999] hover:bg-[#1A1A1A]' : 'border-[#2E2E2E]/20 text-[#666666] hover:bg-[rgba(215,183,151,0.1)]'
                }`}
              >
                Cancel
              </button>
